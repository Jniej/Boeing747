...
Kp_pitch_landing = 1.2;
Ki_pitch_landing = 0.03;
Kd_pitch_landing = 0.7;
...
touchdown_index = 3000;
landing_started      = false;
on_runway_land       = false;
V_land               = 0;
pitch_error_int_land = 0;
prev_pitch_error_land= 0;
speed_error_int_land = 0;
prev_speed_error_land= 0;
...
%         print("descent")
    ...
    elseif landing_stage 
        if ~landing_started
            landing_started       = true;
            V_land                = V_cruise;    % starting speed at top of approach
            pitch_error_int_land  = 0;
            prev_pitch_error_land = 0;
            speed_error_int_land  = 0;
            prev_speed_error_land = 0;
        ...
        if on_runway_land
            ...
            decel       = 1.21;               % m/s²
            pitch_decay = deg2rad(2);         % rad/s
            V_land      = max(0, V_land - decel * dt);
            pitch_angle(i) = max(0, pitch_angle(i-1) - pitch_decay * dt);

            % keep at runway altitude
            Z(i) = runway_altitude;
            X(i) = X(i-1) + V_land * cos(pitch_angle(i)) * dt * cos(heading_angle(i-1));
            Y(i) = Y(i-1) + V_land * cos(pitch_angle(i)) * dt * sin(heading_angle(i-1));

            if V_land <= 0.1
                % fully stopped → end simulation
                touchdown_index = i;   % record where we stopped
                break;
            end
            
%             fprintf('t = %.1f s,   V_land = %.2f m/s\n', t(i), V_land);
            

        else
            % --- Approach & Flare Phases ---
            current_alt = Z(i-1);
            % pick desired pitch & speed
            if current_alt > flare_altitude
                % approach
                desired_pitch = deg2rad(-3);
                desired_speed = approach_speed - ...
                    (approach_speed - touchdown_speed) * ...
                    (1 - min(1,(current_alt - flare_altitude)/(initial_altitude - flare_altitude)));
                max_sink = 8; min_sink = 5;
                desired_sink = -min_sink - (max_sink-min_sink)*(current_alt-flare_altitude)/(initial_altitude-flare_altitude);
            else
                % flare
                prog = min(1, current_alt/flare_altitude);
                desired_pitch = deg2rad(-3 + 8*(1-prog));
                desired_speed = touchdown_speed + (approach_speed - touchdown_speed)*prog*0.3;
                desired_sink  = -max(0.2,0.5*prog);
            end

            % --- Speed control (PI‑D) ---
            speed_err = desired_speed - V_land;
            speed_error_int_land = speed_error_int_land + speed_err*dt;
            speed_err_deriv      = (speed_err - prev_speed_error_land)/dt;
            speed_cmd = Kp_speed*speed_err + Ki_speed*speed_error_int_land + Kd_speed*speed_err_deriv;
            max_accel = 0.8;
            V_land = V_land + sign(speed_cmd)*min(abs(speed_cmd), max_accel*dt);
            prev_speed_error_land = speed_err;

            % --- Pitch control (PI‑D) ---
            if current_alt < flare_altitude
                req_pitch = asin(desired_sink / V_land);
                desired_pitch = max(desired_pitch, req_pitch);
            end
            pitch_err = desired_pitch - pitch_angle(i-1);
            pitch_error_int_land  = pitch_error_int_land + pitch_err*dt;
            pitch_err_deriv       = (pitch_err - prev_pitch_error_land)/dt;
            pitch_cmd = Kp_pitch_landing*pitch_err ...
                      + Ki_pitch_landing*pitch_error_int_land ...
                      + Kd_pitch_landing*pitch_err_deriv;
            max_prate = deg2rad(4);
            pitch_angle(i) = pitch_angle(i-1) + sign(pitch_cmd)*min(abs(pitch_cmd), max_prate*dt);
            prev_pitch_error_land = pitch_err;

            % --- Update position & altitude ---
            X(i) = X(i-1) + V_land*cos(pitch_angle(i))*dt*cos(heading_angle(i-1));
            Y(i) = Y(i-1) + V_land*cos(pitch_angle(i))*dt*sin(heading_angle(i-1));
            new_alt = Z(i-1) + V_land*sin(pitch_angle(i))*dt;
            if new_alt < runway_altitude
                Z(i) = runway_altitude;
                on_runway_land = true;
%                 fprintf('horray');
            else
            ...
            % to print once per time‑step in your loop:
%             fprintf('t = %.1f s,   V_land = %.2f m/s\n', t(i), V_land);
...
t            = t(1:touchdown_index);
X            = X(1:touchdown_index);
Y            = Y(1:touchdown_index);
Z            = Z(1:touchdown_index);
pitch_angle  = pitch_angle(1:touchdown_index);
heading_angle= heading_angle(1:touchdown_index);
bank_angle   = bank_angle(1:touchdown_index);

pitch_error_log   = pitch_error_log(1:touchdown_index);
P_pitch_log       = P_pitch_log(1:touchdown_index);
I_pitch_log       = I_pitch_log(1:touchdown_index);
D_pitch_log       = D_pitch_log(1:touchdown_index);

heading_error_log = heading_error_log(1:touchdown_index);
P_term_log        = P_term_log(1:touchdown_index);
I_term_log        = I_term_log(1:touchdown_index);
D_term_log        = D_term_log(1:touchdown_index);

gust_heading = gust_heading(1:touchdown_index);
gust_pitch   = gust_pitch(1:touchdown_index);
gust_r       = gust_r(1:touchdown_index);
